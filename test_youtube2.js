(function () {
'use strict';
// Версія 1.54.4: Покращено tryAddMenu: 10 спроб, затримка 1000 мс, додаткові логи для діагностики Lampa.Menu
...
function tryAddMenu(attempt, maxAttempts) {
    console.log('TryAddMenu attempt:', attempt, 'Lampa.Menu:', Lampa.Menu, 'Lampa.Manifest:', Lampa.Manifest, 'Lampa.Manifest.app_digital:', Lampa.Manifest?.app_digital, 'Keys:', Object.keys(Lampa.Menu || {}));
    if (attempt >= maxAttempts) {
        console.error('Lampa.Menu.add failed after', maxAttempts, 'attempts. Final Lampa.Menu:', Lampa.Menu, 'Lampa.Manifest:', Lampa.Manifest, 'Keys:', Object.keys(Lampa.Menu || {}));
        console.error('Please check Lampa version compatibility (expected app_digital >= 166) and ensure Lampa.Menu is properly initialized.');
        return;
    }
    if (Lampa.Menu && typeof Lampa.Menu.add === 'function') {
        ...
        console.log('Lampa.Menu.add executed successfully on attempt:', attempt, 'Lampa.Manifest.app_digital:', Lampa.Manifest?.app_digital);
    } else {
        console.warn('Lampa.Menu.add not available on attempt:', attempt, 'Lampa.Menu:', Lampa.Menu, 'Lampa.Manifest:', Lampa.Manifest, 'Keys:', Object.keys(Lampa.Menu || {}));
        setTimeout(function () {
...
Lampa.Listener.follow('app', function (e) {
    if (e.type === 'ready') {
        console.log('App ready. Lampa.Manifest:', Lampa.Manifest, 'Lampa.Manifest.app_digital:', Lampa.Manifest?.app_digital, 'Lampa.Menu:', Lampa.Menu, 'Keys:', Object.keys(Lampa.Menu || {}));
        tryAddMenu(1, 10); // Збільшено до 10 спроб
    }
    if (e.type === 'start') {
        var lang = Lampa.Storage.get('language', 'ru');
