(function () {
    ...
    function initPlugin() {
        ...
        var style = $('<style>' +
            ...
            '.orientation--portrait:not(.size--big) .card--new_seria { position: absolute; top: 3.2em; right: 0.5em; width: auto; max-width: 50%; font-size: 1em; padding: 0.15em 0.4em; border-radius: 0.5em; z-index: 11; background: rgba(0,0,0,0.3); }' +
            '.orientation--landscape .full-start-new__left, .size--big .full-start-new__left { display: flex; flex-direction: column; align-items: flex-end; }' +
            '.orientation--landscape .card--new_seria, .size--big .card--new_seria { width: auto; margin-top: 0.3em; margin-bottom: 0.5em; }' +
            '</style>');
        $('head').append(style);
