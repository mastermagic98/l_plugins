(function () {
    ...
    function get(url, page, resolve, reject) {
        var full_url = tmdb_base_url + url + '&api_key=' + tmdb_api_key + '&page=' + page + '&language=' + window.plugin_upcoming.utils.getInterfaceLanguage();
    ...
    function getLocalMoviesInTheaters(page, resolve, reject) {
        ...
        var language = window.plugin_upcoming.utils.getInterfaceLanguage();
        var today = new Date();
    ...
    function getUpcomingMovies(page, resolve, reject) {
        var region = window.plugin_upcoming.utils.getRegion();
        var today = window.plugin_upcoming.utils.getFormattedDate(0);
        var sixMonthsLater = window.plugin_upcoming.utils.getFormattedDate(-180);
        var language = window.plugin_upcoming.utils.getInterfaceLanguage();
    ...
    function main(oncomplite, onerror) {
        ...
        var today = window.plugin_upcoming.utils.getFormattedDate(0);
        var sixMonthsLater = window.plugin_upcoming.utils.getFormattedDate(-180);
        var threeMonthsAgo = window.plugin_upcoming.utils.getFormattedDate(90);
        var threeMonthsLater = window.plugin_upcoming.utils.getFormattedDate(-90);
        ...
        if (params.type === 'in_theaters') {
            ...
            if (cachedData && cachedData.results && cachedData.results.length > 0) {
                var filteredResults = cachedData.results.filter(function (m) {
                    if (m.release_details && m.release_details.results) {
                        var regionRelease = m.release_details.results.find(function (r) {
                            return r.iso_3166_1 === window.plugin_upcoming.utils.getRegion();
                        });
                        if (regionRelease && regionRelease.release_dates && regionRelease.release_dates.length) {
                            return true;
                        }
