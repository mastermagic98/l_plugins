...
(function () {
	...
	var TRASH_R = ['$$$####!!!!!!!', '^^^^^^##@', '@!^^!@#@@$$$$$', '^^#@@!!@#!$', '@#!@@@##$$@@'];
	var version_modss = '3.1', API = Protocol() + 'api.lampa.stream/', type = '', jackets = {}, cards, ping_auth, manifest, menu_list = [], vip = true, leftVipD = 99999, user_id = 900000000, uid = '0000000x000x000000x0xx0x0_000000000', IP = '2000:0000:0000:9b00:0000:0000:0000:ad8f', logged = true;
      ...
      this.tv_modss();
        ...
        setTimeout(function (){
          $('.extensions__item-author', Lampa.Extensions.render()).map(function (i, e){
            if(e.textContent == '@modss_group') $(e).html('🍞').append('<span class="extensions__item-premium">VIP hacked by Xackerr</span>');
          });
        }, 500);
      });
    ...
    sources: function () {
      ...
      Lampa.Modal.render().find('.cub-premium__title').text("MODS's VIP Plus Pro Max 2TB");
      Lampa.Modal.render().find('.cub-premium__descr:eq(0)').text('Поздравляем вас с установкой плагина! Теперь у вас есть возможность наслаждаться видео в высоком разрешении 4К. Кроме того, вас ожидают многие балансеры, которые помогут найти подходящий контент');
      Lampa.Modal.render().find('.cub-premium__descr:eq(1)').text('Подписка: <b style="color: #ffd402;>Безлимит++</b>');
      Lampa.Modal.render().find('.cub-premium__descr:eq(1)').after('👇 Кнопка для просмотра 👇<br><br><img src="http://lampa.stream/but_modss.jpg">');
      Lampa.Modal.render().find('.cub-premium__url').text('@yes');
	  ...
	  rating_kp_imdb:function (card) {
        ...
        Pub.network.clear();
          ...
          var kp_rating = !isNaN(kp) && kp !== null ? parseFloat(kp).toFixed(1) : '0.0';
  			  ...
  			  user_id: user_id, 
  			  uid: uid,
  			  ips: '2000:0000:0000:9b00:0000:0000:0000:ad8f',
    ...
    auth: function(kp) {
      ...
      return {
		...
		balansers: function() {
		  var balansers = {"hdr":"MODS's [4K, HDR]  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kinopub":"KinoPub 4K","filmix":"Filmix","alloha":"Alloha 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","hdrezka":"HDRezka 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","zetflix":"Zetflix  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","uakino":"UAKino 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","eneida":"Eneida(Ukr)  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kodik":"Kodik  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","anilibria":"Anilibria  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","hdvb":"HDVB  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kinobase":"Kinobase","cdnmovies":"CDNmovies","videocdn":"VideoCDN","rezka":"Voidboost","collaps":"Collaps","kinotochka":"KinoTochka  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","mango":"ManGo 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>","kinofit":"KinoFit 4K  <span style=\"font-weight: 700;color:rgb(236,151,31)\">VIP</span>"};
	...
	var Pub = {
  	...
  	tock: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx',
  	token: Lampa.Storage.get('pub_access_token', 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx'),
  	openBrowser: function (url) {
  		if (Lampa.Platform.is('tizen')) {
  		...
  		if (!Pub.token) status.removeClass('wait').addClass('error');
  			...
  			this.network.silent(Pub.baseurl + 'v1/user?access_token=' + Pub.token, function (json) {
  				if(json.user.username.indexOf('MODSS') == -1) {
					$('.settings-param__' + (!ur ? 'name' : 'descr'), itm).html('<img width="30em" src="' + json.user.profile.avatar + '">  <span style="vertical-align: middle;"><b style="font-size:1.4em;color:#FF8C00">' + '</b> - ' + 'Подписка <b style="color: #ffd402;>Безлимит++</b>' + '</span>');
  ...
  function component(object) {
    ...
    var filter_sources = {};
    var localBal = ['videocdn', 'rezka', 'cdnmovies'];
    ...
    this.requestParams = function (url) {
      ...
      query.push('cub_id=eGFja3IzMkBnbWFpbC5jb20=');
      query.push('uid=deae144f041e98948dda03e33_653566559');
      if (cards) {
				var notices = Lampa.Storage.get('account_notice', []).filter(function (n) {
        ...
        xhr.onreadystatechange = function() {
          if (xhr.readyState === 2) {
            if (xhr.status >= 200 && xhr.status < 300) {
              console.log('Modss','checkHash',' - OK');
              resolve(params);
    ...
    this.getFileUrl = function (file, call) {
      ...
      if (balanser == 'qiwi') {
        ...
        if (file.url && ['hdrezka','zetflix','videodb'].indexOf(balanser) >= 0  && 'DE' == 'UA') {
        ...
        network["native"](file.url, function (json) {
          if (json.url && ['hdrezka','kodik'].indexOf(balanser) >= 0 && ['UA'].indexOf('DE') >= 0) {
    ...
    this.filter = function (filter_items, choice) {
      var _this6 = this;
      var select = [];
      var countElements = function countElements(data,s) {
        var result = {};
      ...
      var add = function add(type, title) {
        ...
        items.forEach(function (name, i) {
          ...
          if(type == 'voice') {
            var S = need.season + 1;
            var counts = countElements(extract.folder, S);
      ...
      this.get = function (){
        if (object.url.indexOf('pub/tvs') > -1) {
    			network.silent(object.url, _this1.build.bind(this), function (json) {
    			  ...
    			  id:'eGFja3IzMkBnbWFpbC5jb20=',
				  uid:'deae144f041e98948dda03e33_653566559'
    			});
          ...
          network['native'](object.url, function (str) {
    	    ...
    	    var pl = [{"name":"Kulik","url":"kulik"}]
          _this1.list(pl);
    		}
      };
      this.parsePlaylist = function (data) {
        var postdata = data ? {datas: data, id:'eGFja3IzMkBnbWFpbC5jb20=',uid:'deae144f041e98948dda03e33_653566559'} : {url: object.url, id:'eGFja3IzMkBnbWFpbC5jb20=',uid:'deae144f041e98948dda03e33_653566559'};
...
interfaceElement.after(settingsElement);
        ...
        Lampa.SettingsApi.addParam({
          ...
          field: {
            name: '😺 MODSS Vip Plus Pro Max 2TB'
          }
        });
			...
			Lampa.SettingsApi.addParam({
				...
				field: {
					name: '<img width="150px" src="https://i.imgur.com/Qdft7dg_d.webp?maxwidth=760&fidelity=grand">',
					description: '<b style="color: #ffd402;">Vip</b> статус взломан пользователем "<b style="color: #ffd402;">Xackerr</b>"'
				}
			});
			Lampa.SettingsApi.addParam({
				component: 'settings_modss',
  ...
  Lampa.SettingsApi.addParam({
      ...
      Filmix.network["native"](u, function (j) {
		...
		window.dataLayer = window.dataLayer || [];
		function gtag() {
			var idl = '2000:0000:0000:9b00:0000:0000:0000:ad8f';
