(function () {
  ...
  // Всегда устанавливаем авторизацию как активную
  var isCodeObtained = true;

  // Удаляем интервал проверки авторизации
  ...
  // Заглушки для функций авторизации
  ...
  function checkCode() {
    // Функция-заглушка, всегда считаем, что код проверен успешно
    return;
  }
  ...
  function component(object) {
    ...
    this.appendAPN = function (data) {
      if (
        Defined.api.indexOf("pwa") == 0 &&
        Defined.apn.length &&
        data.url &&
        typeof data.url == "string" &&
        data.url.indexOf(Defined.apn) == -1
      )
  ...
  function startPlugin() {
    ...
    var button =
      '<div class="full-start__button selector view--online_showyPROEU showyPROEU--button" data-subtitle="'
  ...
  Lampa.Listener.follow("full", function (e) {
    if (e.type == "complite") {
      setTimeout(function () {
        $(".view--online_showyPROEU", Lampa.Activity.active().activity.render())
          .empty()
          .append(
