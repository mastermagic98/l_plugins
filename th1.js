(function() {
    ...
    function applyTheme(theme, color) {
        ...
        var svgCode = encodeURIComponent(
            '<svg xmlns="http://www.w3.org/2000/svg" width="135" height="140" fill="' + selectedColor + '">' +
        ...
        // Створюємо новий стиль (окремо від інших стилів)
        var styleEl = $('<style id="interface_mod_theme"></style>');
        ...
        try {
            styleEl.html(dynamicTheme);
            $('head').append(styleEl);
        } catch (e) {}
    }

    // Функція для управління класами кнопок (з підтримкою динамічно доданих елементів)
    ...
    function applyButtonStylesToElement(el) {
        try {
            ...
            if (SafeStyle.settings.show_all_buttons) {
                ...
                // маркуємо елемент, щоб знати, що ми його обробили
                $el.attr('data-safe-style', '1');
            } else {
                // прибираємо класи тільки якщо ми їх додавали раніше (щоб не торкатися сторонніх модифікацій)
        ...
        try {
            ...
            _buttonObserver = new MutationObserver(function(mutations) {
                mutations.forEach(function(mutation) {
                        ...
                        for (var i = 0; i < mutation.addedNodes.length; i++) {
                            ...
                            try {
                                if ($(node).is(_buttonSelector)) {
                                    ...
                                    var found = $(node).find(_buttonSelector);
                                    if (found.length) {
                                        found.each(function() {
                                            applyButtonStylesToElement(this);
                                        });
                    ...
                    // обробляємо зміни атрибутів — можливі випадки, коли клас додається пізніше
            ...
            if (target) {
                _buttonObserver.observe(target, { childList: true, subtree: true, attributes: true, attributeFilter: ['class'] });
            }
        } catch (e) {}
    }
    ...
    function updateButtonStyles() {
        try {
            if (SafeStyle.settings.show_all_buttons) {
                // застосувати до існуючих
                applyButtonStylesToExisting();
                // і запускати спостерігача для динамічно доданих елементів
                startButtonObserver();
            } else {
                // прибрати клас лише з тих елементів, які були змарковані нашим плагіном
                $(_buttonSelector).each(function() {
                    try {
                        var $el = $(this);
                        if ($el.attr('data-safe-style')) {
    ...
    function AddIn() {
        ...
        var baseStyle = ''
            ...
            + ' .card__title { height: 3.6em; text-overflow: ellipsis; -o-text-overflow: ellipsis; text-overflow: ellipsis; -webkit-line-clamp: 3; line-clamp: 3; }'
            ...
            + ' .card__type { position: absolute; bottom: auto; left: 0em; right: auto; top: 0em; background: rgba(0,0,0,0.6); color:#fff; font-weight:700; padding:0.4em 0.6em; border-radius:0.4em 0 0.4em 0; line-height:1.0; font-size:1.0em; }'
            + ' .card--tv .card__type { color:#fff; }'
            + ' .card__icons { position: absolute; top: 2em; left: 0; right: auto; display:flex; justify-content:center; background: rgba(0,0,0,0.6); color:#fff; border-radius:0 0.5em 0.5em 0; }'
            + ' .card__icons-inner { background: rgba(0,0,0,0); }'
            + ' .card__marker { position:absolute; left:0em; top:4em; bottom:auto; background: rgba(0,0,0,0.6); border-radius:0 0.5em 0.5em 0; font-weight:700; font-size:1.0em; padding:0.4em 0.6em; display:flex; align-items:center; line-height:1.2; max-width:min(12em,95%); box-sizing:border-box; }'
            + ' .card__marker > span { max-width: min(12em,95%); }'
            + ' .card__quality { position:absolute; left:auto; right:0em; bottom:2.4em; padding:0.4em 0.6em; color:#fff; font-weight:700; font-size:1.0em; border-radius:0.5em 0 0 0.5em; text-transform:uppercase; }'
            ...
            + ' .extensions__item, .extensions__block-add, .search-source, .bookmarks-folder__layer, .bookmarks-folder__body, .card__img, .card__promo, .full-episode--next .full-episode__img:after, .full-episode__img img, .full-episode__body, .full-person__photo, .card-more__box, .full-start__button, .simple-button, .register { border-radius: 0.5em; }'
            ...
            + ' .card { transform: scale(1); transition: transform 0.3s ease; } .card.focus { transform: scale(1.03); }'
            + ' .torrent-item, .online-prestige { transform: scale(1); transition: transform 0.3s ease; } .torrent-item.focus, .online-prestige.focus { transform: scale(1.01); }'
            + ' .tag-count, .full-person, .full-episode, .simple-button, .full-start__button, .items-cards .selector, .card-more, .explorer-card__head-img.selector, .card-episode { transform: scale(1); transition: transform 0.3s ease; }'
            + ' .tag-count.focus, .full-person.focus, .full-episode.focus, .simple-button.focus, .full-start__button.focus, .items-cards .selector.focus, .card-more.focus, .explorer-card__head-img.selector.focus, .card-episode.focus, .card-episode.hover { transform: scale(1.02); }'
        ...
        if (Lampa.SettingsApi) {
            Lampa.SettingsApi.addComponent({
                ...
                name: Lampa.Lang ? (Lampa.Lang.translate ? Lampa.Lang.translate('Safe Style') : 'Safe Style') : 'Safe Style',
                icon: ''
        ...
        if (Lampa.SettingsApi) {
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: Lampa.Lang ? (Lampa.Lang.translate ? Lampa.Lang.translate('Тема') : 'Тема') : 'Тема',
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: Lampa.Lang ? (Lampa.Lang.translate ? Lampa.Lang.translate('Колір теми') : 'Колір теми') : 'Колір теми',
            ...
            // === ДОДАНА ОПЦІЯ: Показувати всі кнопки (включити/вимкнути) ===
            Lampa.SettingsApi.addParam({
                component: 'safe_style',
                param: {
                ...
                field: {
                    name: Lampa.Lang ? (Lampa.Lang.translate ? Lampa.Lang.translate('Показувати всі кнопки') : 'Показувати всі кнопки') : 'Показувати всі кнопки',
            ...
            Lampa.SettingsApi.addParam({
                ...
                field: {
                    name: Lampa.Lang ? (Lampa.Lang.translate ? Lampa.Lang.translate('Увімкнути плагін') : 'Увімкнути плагін') : 'Увімкнути плагін',
        ...
        // Застосовуємо стилі кнопок одразу після ініціалізації AddIn
        try { updateButtonStyles(); } catch (e) {}
    }
    ...
    if (Lampa && Lampa.Listener && Lampa.Listener.follow) {
        Lampa.Listener.follow('app', function(e) {
            try {
                if (e.type === 'ready') {
                    // Завантажуємо збережені налаштування (безпечні спроби)
                    ...
                    try { SafeStyle.settings.enabled = (typeof Lampa.Storage.get('safe_style_enabled') !== 'undefined') ? Lampa.Storage.get('safe_style_enabled') : true; } catch (er) {}
                    try { SafeStyle.settings.show_all_buttons = (typeof Lampa.Storage.get('safe_style_show_all_buttons') !== 'undefined') ? Lampa.Storage.get('safe_style_show_all_buttons') : false; } catch (er) {}

                    // Застосовуємо шаблони та стилі з невеликою затримкою, щоб UI встиг ініціалізуватися
                    setTimeout(function() {
                        try {
                            if (Lampa.SettingsApi) {
                                AddIn();
                                applyTheme(SafeStyle.settings.theme, SafeStyle.settings.custom_color);
                                // впевнюємось що стилі кнопок застосовані після ініціалізації DOM
                                updateButtonStyles();
                            }
                        } catch (err) {}
                    }, 100);
