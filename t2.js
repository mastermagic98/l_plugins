(function () {
    'use strict';
    // Версія 1.51: Додано дату прем'єри для категорії "Очікувані фільми" та видалено логи
                        ...
                        onSelect: function (item) {
                            ...
                            Lampa.Activity.push({
                                url: item.value === 'day' ? '/discover/' + (data.type.includes('movie') ? 'movie' : 'tv') + '?sort_by=popularity.desc' :
                                     item.value === 'week' ? '/discover/' + (data.type.includes('movie') ? 'movie' : 'tvVs') + '?sort_by=popularity.desc' :
