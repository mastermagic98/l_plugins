(function () {  
    ...
    function applyHiding() {  
        LANGUAGES.forEach(lang => {  
            ...
            if (element.length) {  
                element.toggle(!hide);  
                console.log('Keyboard Hide: ' + lang + (hide ? ' hidden' : ' shown'));  
            } else {  
        ...
        onRender(el) {  
            el.off('hover:enter').on('hover:enter', function () {  
                Lampa.Select.show({  
                    ...
                    onSelect(item) {  
                        if (item.title === 'Так') openLanguageMenu();  
                    },  
                    onBack() {  
                        Lampa.Controller.toggle('settings_component');  
                    }  
